...
class FilesystemStore {
  ...
  async getMetadata(bucket, key) {
    ...
    // deta
    const stat = await drive.list({prefix: objectPath})

    const storedMetadata = JSON.parse(await drive.get(metadataPath));
    var md5 = (await drive.get(`${objectPath}.md5`)).toString();
    if (!md5){
          ...
          await deta.put(`${objectPath}.md5`, {data: md5});
    }
    return {
      ...storedMetadata,
  ...
  async putMetadata(bucket, key, metadata, md5) {
    ...
    if (md5) await drive.put(md5Path, {data: md5});
    await drive.put(metadataPath, { data: JSON.stringify(json, null, 2)});
  }
  ...
  async listBuckets() {
    ...
    */
    ...
    return process.env.BUCKETS; // return config array
  ...
  async deleteBucket(bucket) {
    ...
    const drive = deta.Drive(bucket);
    // Do we need to iterate the file list and delete all of them?
    const deletedFile = await drive.delete("*");
    return true
  }
  ...
  async existsObject(bucket, key) {
    ...
    */
    ...
    const drive = deta.Drive(bucket);
    const exists = await drive.list({prefix: "key"})
  ...
  async deleteObject(bucket, key) {
    ...
    // WIP 
    const drive = deta.Drive(bucket);
    const result = await drive.DeleteMany(["object", "object.md5", "metadata.json"]);
    /*
    await Promise.all(
      [
        this.getResourcePath(bucket, key, 'object'),
