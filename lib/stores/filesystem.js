...
class FilesystemStore {
  ...
  */
  ...
  async listBuckets() {
    ...
    const drive = deta.Drive('tmp');
    if (debug) console.log(this.rootDirectory)
    const list = await fs.readdir(this.rootDirectory);
    if (debug) console.log("readdir")
    if (debug) console.log(list)
    const dlist = await drive.list({prefix: this.rootDirectory}).catch((err)=>{console.log(err)})
    if (debug) console.log("readdir")
    if (debug) console.log(dlist)
    if (debug) console.log("list, dlist", list, dlist)
    const buckets = await Promise.all(
      dlist.names.map((filename) => this.getBucket(filename)),
    );
  ...
  async getBucket(bucket) {
    const drive = deta.Drive(bucket);
    if (debug) console.log("getBucket called with ", bucket)
    const bucketPath = this.getBucketPath(bucket);
    try {
      const blob = await drive.get("stat")
      const obj = await blob.text()
      var stat = JSON.parse(obj)
    } catch (err) {
      if (debug) console.log("getBucket Error:", err)
      return null
    }
  ...
  async putBucket(bucket) {
    const drive = deta.Drive(bucket);
    if (debug) console.log(await drive.list())
    ...
    await drive.put('stat', {data: json})
